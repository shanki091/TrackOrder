trackOrder.factory('trackOrderService', function($http) {
	  var urls = {
		 getTrackOrder : 'https://www.clickpost.in/api/v2/track-order/?username=testuser&key=2e9b19ac-8e1f-41ac-a35b-4cd23f41ae17'	 // getTrackOrder 
	  }
   ...
   return {
		getTrackOrder : function(waybill, cpId) {
			return $http({
				method : 'GET',
				url : urls.getTrackOrder + '&waybill=' + waybill + '&cp_id=' + cpId
			});
		},
		getStandardDate : function (timeStamp)  {
			var date = new Date(timeStamp);
            return DateFormat(date, "mmm d, yyyy", false);
		},
		getStandardTime : function (timeStamp)  {
			var date = new Date(timeStamp);
            return DateFormat(date, "h:MM TT", false);
		}
   }
});
